...
display_results() {
  ...
  if [ "$SVC_PRE" = "systemctl" ]; then
    ...
    info "Supervisor Start Command:      $(fg_cyan "sudo startsrc -s observiq-otel-collector -a start -e "$(cat /opt/observiq-otel-collector/observiq-otel-collector.env)"")$(reset)"
    info "Supervisor Stop Command:       $(fg_cyan "sudo stopsrc -s observiq-otel-collector")$(reset)"
    info "Supervisor Status Command:     $(fg_cyan "sudo lssrc -s observiq-otel-collector")$(reset)"
  fi
  decrease_indent
