...
.PHONY: agent
agent:
	go build -ldflags "-s -w -X github.com/observiq/bindplane-agent/version.version=$(VERSION)" -o $(OUTDIR)/collector_$(GOOS)_$(GOARCH)$(EXT) ./cmd/collector

# Builds just the updater for current GOOS/GOARCH pair
.PHONY: updater
updater:
	cd ./updater/; go build -ldflags "-s -w -X github.com/observiq/bindplane-agent/version.version=$(VERSION)" -o ../$(OUTDIR)/updater_$(GOOS)_$(GOARCH)$(EXT) ./cmd/updater
